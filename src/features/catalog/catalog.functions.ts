...
export const createResearchExport = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    const records = [...latest.values()].map((row) => ({ product: { productKey: row.product.id, title: row.product.title, description: row.product.description ?? undefined, brand: row.product.brand ?? undefined, productType: row.product.productType ?? undefined, tags: row.product.tags }, variant: { variantKey: row.variant.id, productKey: row.product.id, title: row.variant.title ?? undefined, sku: row.variant.sku ?? undefined, barcode: row.variant.barcode ?? undefined, available: row.current.available, price: row.current.price ? Number(row.current.price) : undefined, compareAtPrice: row.current.compareAtPrice ? Number(row.current.compareAtPrice) : undefined, currency: row.current.currency ?? undefined }, listing: { listingKey: row.listing.id, sourceKey: row.source.sourceKey, productKey: row.product.id, variantKey: row.variant.id, url: row.listing.url, imageUrl: row.listing.imageUrl ?? undefined, current: { title: row.current.title, price: row.current.price ? Number(row.current.price) : undefined, compareAtPrice: row.current.compareAtPrice ? Number(row.current.compareAtPrice) : undefined, currency: row.current.currency ?? undefined, available: row.current.available }, observedAt: row.current.observedAt.toISOString() }, evidenceId: row.evidence?.id }))
