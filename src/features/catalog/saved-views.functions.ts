...
export const saveListingView = createServerFn({ method: 'POST' })
  .validator(z.object({ name: savedViewNameSchema, filters: listingFiltersSchema, presentation: listingPresentationSchema }))
  .handler(async ({ data }) => {
    const session = await requireSession()
    setResponseHeader('Cache-Control', 'private, no-store')
    return saveListingViewToDatabase(getDatabase(), session.user.id, data.name, data.filters, data.presentation)
  })
