...
export function CurrentListingsTable({ listingPage, savedViews, watchedListingIds = [], researchSummaries = {}, workbench }: { listingPage: CurrentListingsPage; savedViews: SavedView[]; watchedListingIds?: string[]; researchSummaries?: ResearchSummariesByListing; workbench: ListingWorkbench }) {
  ...
  return <div>
    <CurrencyFilter value={filters.currency} onChange={(value) => updateFilter('currency', value)} />
    <label className="ml-2 text-xs text-muted-foreground">Presence <select aria-label="Presence" value={filters.presence} onChange={(event) => updateFilter('presence', event.target.value as ListingFilters['presence'])} className="h-8 rounded border border-border bg-background px-1.5 text-xs text-foreground"><option value="present">Present</option><option value="missing">Missing</option><option value="all">All</option></select></label>
