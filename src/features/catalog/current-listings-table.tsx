import { createColumnHelper, rowSortingFeature, tableFeatures, useTable } from '@tanstack/react-table'
import { Link, useNavigate, useRouter } from '@tanstack/react-router'
import { useServerFn } from '@tanstack/react-start'
import { useEffect, useMemo, useRef, useState } from 'react'
import { useDebouncer } from '@tanstack/react-pacer'
import { createResearchExport } from './catalog.functions'
import type { CurrentListing, CurrentListingsPage } from './catalog.schemas'
import { emptyListingFilters, listingDiscount, listingFiltersSchema, type ListingFilters } from './listing-filters'
...
import { type listingPresentationSchema, type listingWorkbenchSearchSchema } from './listing-workbench-state'
import type { z } from 'zod'

const features = tableFeatures({ rowSortingFeature, manualSorting: true })
...
type SavedView = { id: string; name: string; filters: ListingFilters; presentation: z.output<typeof listingPresentationSchema> }
...
function CurrencyFilter({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  return <label className="flex items-center gap-1 whitespace-nowrap text-xs text-muted-foreground">Currency<input aria-label="Filter currency" inputMode="text" maxLength={3} value={value} onChange={(event) => onChange(event.target.value.toUpperCase())} className="h-8 w-14 rounded border border-border bg-background px-1.5 text-xs uppercase text-foreground" placeholder="Any" /></label>
}

type ListingWorkbench = z.output<typeof listingWorkbenchSearchSchema>

export function CurrentListingsTable({ listingPage, savedViews, watchedListingIds = [], researchSummaries = {}, workbench }: { listingPage: CurrentListingsPage; savedViews: SavedView[]; watchedListingIds?: string[]; researchSummaries?: ResearchSummariesByListing; workbench: ListingWorkbench }) {
  const { listings, facets, total, pageCount } = listingPage
  ...
  const queryDebouncer = useDebouncer((value: string) => {
    void navigate({ search: (current) => ({ ...current, filters: { ...current.filters, query: value }, page: 0 }) })
  }, { wait: 250 })
  useEffect(() => { setQueryDraft(filters.query) }, [filters.query])
  useEffect(() => {
    if (workbench.page >= pageCount) void navigate({ search: (current) => ({ ...current, page: pageCount - 1 }), replace: true })
  }, [navigate, workbench.page, pageCount])
  const watched = useMemo(() => new Set(watchedListingIds), [watchedListingIds])
  const visibleCategories = useMemo(() => facets.categories
    .filter((category) => !filters.categoryGroup || category.categoryGroups.includes(filters.categoryGroup))
    .map((category) => category.value), [facets.categories, filters.categoryGroup])
  function updateFilter<K extends keyof ListingFilters>(key: K, value: ListingFilters[K]) {
    ...
    const nextFilters = {
      ...
      query: queryDraft,
      [key]: value,
      ...(key === 'categoryGroup' && value && filters.category && !facets.categories.some((category) => category.value === filters.category && category.categoryGroups.includes(String(value))) ? { category: '' } : {}),
    }
  ...
  function loadView(id: string) {
    ...
    void navigate({ search: (current) => ({ ...current, filters: view?.filters ?? emptyListingFilters, sorting: view?.presentation.sorting ?? [], pageSize: view?.presentation.pageSize ?? 50, page: 0 }) })
    setViewError('')
  }
  async function saveCurrentView() {
    setViewBusy(true); setViewError('')
    try {
      const result = await saveView({ data: { name: viewName, filters: listingFiltersSchema.parse(filters), presentation: { sorting: workbench.sorting, pageSize: workbench.pageSize } } })
  ...
  const table = useTable({ features, data: listings, columns: makeColumns(selected, toggle, showVariants, showModules, showMedia, moveMedia, (key, value) => updateFilter(key, value), watched, researchSummaries, showResearch), getRowId: (listing) => listing.id, state: { sorting: workbench.sorting }, onSortingChange: (next) => { const sorting = typeof next === 'function' ? next(workbench.sorting) : next; void navigate({ search: (current) => ({ ...current, sorting, page: 0 }) }) } })
  const rows = table.getRowModel().rows
  const safePage = Math.min(workbench.page, pageCount - 1)
  const pageRows = rows
  const pageSelected = pageRows.length > 0 && pageRows.every((row) => selected.has(row.original.id))
  const hasActiveFilters = (Object.keys(emptyListingFilters) as Array<keyof ListingFilters>).some((key) => filters[key] !== emptyListingFilters[key])
  function togglePageSelection() { setSelected((current) => { const next = new Set(current); for (const row of pageRows) { if (pageSelected) next.delete(row.original.id); else next.add(row.original.id) } return next }) }
  return <div>
    {(filters.minPrice !== null || filters.maxPrice !== null || filters.minDiscountAmount !== null || filters.maxDiscountAmount !== null) && !filters.currency ? <p className="text-xs text-amber-600">Select a currency for absolute price/discount filters.</p> : null}
    ...
    <div className="mb-1 flex flex-wrap items-center gap-1.5"><div className="min-w-52 flex-[2_1_16rem]"><Input aria-label="Search current listings" className="h-8 text-xs" placeholder="Search title, SKU, tag…" value={queryDraft} onChange={(event) => { setQueryDraft(event.target.value); queryDebouncer.maybeExecute(event.target.value) }} onBlur={() => queryDebouncer.flush()} onKeyDown={(event) => { if (event.key === 'Enter') queryDebouncer.flush() }} /></div><select aria-label="Category groups" value={filters.categoryGroup} onChange={(event) => updateFilter('categoryGroup', event.target.value)} className="h-8 min-w-36 flex-1 rounded border border-border bg-background px-1.5 text-xs text-foreground"><option value="">All groups</option>{categoryGroups.map((group) => <option key={group.id} value={group.id}>{group.label} ({facets.categoryGroupCounts[group.id] ?? 0})</option>)}<option value={unmappedCategoryGroup.id}>{unmappedCategoryGroup.label} ({facets.categoryGroupCounts[unmappedCategoryGroup.id] ?? 0})</option></select><FilterSelect label="Categories" value={filters.category} values={visibleCategories} onChange={(value) => updateFilter('category', value)} /><FilterSelect label="Manufacturers" value={filters.manufacturer} values={facets.manufacturers} onChange={(value) => updateFilter('manufacturer', value)} /><select aria-label="Sources" value={filters.sourceId} onChange={(event) => updateFilter('sourceId', event.target.value)} className="h-8 min-w-28 flex-1 rounded border border-border bg-background px-1.5 text-xs text-foreground"><option value="">All sources</option>{facets.sources.map(({ id, name }) => <option key={id} value={id}>{name}</option>)}</select><select aria-label="Stock" value={filters.stock} onChange={(event) => updateFilter('stock', event.target.value as ListingFilters['stock'])} className="h-8 rounded border border-border bg-background px-1.5 text-xs text-foreground"><option value="all">All stock</option><option value="in">In stock</option><option value="out">Out</option></select></div>
    <div className="mb-1 flex min-h-8 flex-wrap items-center gap-1.5 text-xs"><button type="button" className="h-7 rounded border border-border px-2" onClick={togglePageSelection}>{pageSelected ? 'Clear page' : 'Select page'}</button>{selected.size ? <button type="button" className="text-primary underline" onClick={() => setSelected(new Set())}>Clear selection</button> : null}{Object.entries({ Search: filters.query, Group: filters.categoryGroup, Category: filters.category, Maker: filters.manufacturer, Stock: filters.stock === 'all' ? '' : filters.stock }).filter(([, value]) => value).map(([label, value]) => <span key={label} className="rounded-full border border-border bg-muted px-2 py-0.5">{label}: {value}</span>)}{hasActiveFilters ? <button type="button" onClick={() => loadView('')} className="text-primary hover:underline">Clear filters</button> : null}<details className="relative"><summary className="cursor-pointer rounded border border-border px-2 py-1">Advanced</summary><div className="absolute left-0 z-20 mt-1 flex w-[calc(100vw-1rem)] flex-wrap gap-x-2 gap-y-1 rounded border border-border bg-card p-2 shadow sm:w-[34rem]"><CurrencyFilter value={filters.currency} onChange={(value) => updateFilter('currency', value)} /><label className="flex items-center gap-1 whitespace-nowrap text-xs text-muted-foreground">Presence <select aria-label="Presence" value={filters.presence} onChange={(event) => updateFilter('presence', event.target.value as ListingFilters['presence'])} className="h-8 rounded border border-border bg-background px-1.5 text-xs text-foreground"><option value="present">Present</option><option value="missing">Missing</option><option value="all">All</option></select></label><NumberFilter label="Price min" value={filters.minPrice} onChange={(value) => updateFilter('minPrice', value)} /><NumberFilter label="Price max" value={filters.maxPrice} onChange={(value) => updateFilter('maxPrice', value)} /><NumberFilter label="Off min" value={filters.minDiscountAmount} onChange={(value) => updateFilter('minDiscountAmount', value)} /><NumberFilter label="Off max" value={filters.maxDiscountAmount} onChange={(value) => updateFilter('maxDiscountAmount', value)} /><NumberFilter label="Off % min" value={filters.minDiscountPercent} max={100} onChange={(value) => updateFilter('minDiscountPercent', value)} /><NumberFilter label="Off % max" value={filters.maxDiscountPercent} max={100} onChange={(value) => updateFilter('maxDiscountPercent', value)} /></div></details><details className="relative ml-auto"><summary className="cursor-pointer rounded border border-border px-2 py-1">Saved views</summary><div className="absolute right-0 z-20 mt-1 flex flex-wrap gap-1 rounded border border-border bg-card p-2 shadow"><select aria-label="Saved views" value={selectedViewId} onChange={(event) => loadView(event.target.value)} className="h-8 max-w-48 rounded border border-border bg-background px-1.5"><option value="">Current filters</option>{savedViews.map((view) => <option key={view.id} value={view.id}>{view.name}</option>)}</select><Input aria-label="View name" className="h-8 w-32 text-xs" maxLength={80} placeholder="View name" value={viewName} onChange={(event) => setViewName(event.target.value)} /><button type="button" disabled={viewBusy || !viewName.trim()} onClick={() => void saveCurrentView()} className="h-8 rounded bg-primary px-2 text-primary-foreground disabled:opacity-50">Save</button>{selectedViewId ? <button type="button" disabled={viewBusy} onClick={() => void removeCurrentView()} className="h-8 rounded border border-border px-2 text-destructive">Delete</button> : null}{viewError ? <span role="alert" className="text-destructive">{viewError}</span> : null}</div></details><span className="whitespace-nowrap text-muted-foreground">{total.toLocaleString()} listings</span></div>
    <div className="overflow-x-auto border border-border"><table className="w-max min-w-full text-left text-xs font-normal"><thead className="sticky top-0 z-10 bg-muted text-muted-foreground"><tr>{table.getHeaderGroups()[0]?.headers.map((header) => { const sortable = header.column.getCanSort(); const direction = header.column.getIsSorted(); return <th aria-sort={direction === 'asc' ? 'ascending' : direction === 'desc' ? 'descending' : undefined} className={`whitespace-nowrap px-1.5 py-1.5 font-medium ${['price','compareAtPrice','discount','quantity'].includes(header.column.id) ? 'text-right' : ''} ${header.column.id === 'image' ? 'w-12 min-w-12' : ''}`} key={header.id}>{sortable ? <button className="hover:text-foreground" type="button" onClick={header.column.getToggleSortingHandler()}>{header.isPlaceholder ? null : <table.FlexRender header={header} />}{({ asc: ' ↑', desc: ' ↓' } as Record<string, string>)[direction as string] ?? ''}</button> : header.isPlaceholder ? null : <table.FlexRender header={header} />}</th>})}</tr></thead><tbody className="divide-y divide-border">{pageRows.map((row) => <tr className="hover:bg-muted/30" key={row.id}>{row.getAllCells().map((cell) => <td className={`whitespace-nowrap px-1.5 py-1 font-normal ${['price','compareAtPrice','discount','quantity'].includes(cell.column.id) ? 'text-right tabular-nums' : ''} ${cell.column.id === 'image' ? 'w-12 min-w-12' : ''}`} key={cell.id}><table.FlexRender cell={cell} /></td>)}</tr>)}</tbody></table></div>
    {total === 0 ? <p className="py-6 text-center text-xs text-muted-foreground">No listings match these filters.</p> : <div className="mt-2 flex items-center justify-end gap-2 text-xs"><span className="text-muted-foreground">{safePage * workbench.pageSize + 1}–{Math.min((safePage + 1) * workbench.pageSize, total)} of {total}</span><button disabled={safePage === 0} onClick={() => void navigate({ search: (current) => ({ ...current, page: safePage - 1 }) })} className="rounded border border-border px-2 py-1 disabled:opacity-40">Previous</button><button disabled={safePage >= pageCount - 1} onClick={() => void navigate({ search: (current) => ({ ...current, page: safePage + 1 }) })} className="rounded border border-border px-2 py-1 disabled:opacity-40">Next</button><select aria-label="Rows per page" value={workbench.pageSize} onChange={(event) => void navigate({ search: (current) => ({ ...current, pageSize: Number(event.target.value) as 50 | 100 | 200, page: 0 }) })} className="rounded border border-border bg-background px-1 py-1">{pageSizes.map((size) => <option key={size} value={size}>{size}/page</option>)}</select></div>}
  </div>
}
