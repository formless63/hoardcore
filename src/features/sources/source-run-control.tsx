...
export function SourceRunControl({ sourceId, run, defaultRequestLimit = 3, disabled = false }: { sourceId: string; run?: CollectionRun; defaultRequestLimit?: number; disabled?: boolean }) {
  ...
  const maxRequestsHelpId = `max-requests-help-${sourceId}`
  ...
  return <div className="grid gap-2 text-xs xl:grid-cols-[minmax(0,1fr)_auto_auto_auto] xl:items-end">
    <div className="flex flex-wrap items-center gap-2 self-center text-muted-foreground">{run ? <><Badge variant={run.status === 'failed' ? 'danger' : run.status === 'succeeded' ? 'success' : 'info'}>{labels[run.status]}</Badge><span>{run.requestCount} used · max {run.requestLimit}</span><span>{run.pageCount} pages · {run.productCount} products</span><time dateTime={run.completedAt?.toISOString() ?? run.createdAt.toISOString()}>{(run.completedAt ?? run.createdAt).toLocaleString()}</time><Link className="text-primary underline-offset-2 hover:underline" to="/runs/$runId" params={{ runId: run.id }}>Log</Link></> : <span>No collection runs</span>}</div>
    <label className="flex items-center gap-1 whitespace-nowrap text-muted-foreground"><span className="group relative cursor-help border-b border-dotted border-current" tabIndex={0} aria-describedby={maxRequestsHelpId}>Max requests<span id={maxRequestsHelpId} role="tooltip" className="pointer-events-none absolute bottom-full left-0 z-40 mb-2 hidden w-64 whitespace-normal rounded border border-border bg-popover p-2 text-left text-xs text-popover-foreground shadow-lg group-hover:block group-focus:block">Hard cap for the entire run, including any configured preflight and retries. The run stops rather than exceeding it. Page wait never replaces minimum pacing or source Retry-After.</span></span><Input aria-label="Maximum requests" className="!h-7 !w-12 !px-1.5" type="number" min={2} max={20} step={1} value={requestLimit} onChange={(event) => setRequestLimit(Number(event.target.value))} /></label>
    <label className="flex items-center gap-1 whitespace-nowrap text-muted-foreground">Page wait (mins)<Input aria-label="Wait between full pages in minutes" className="!h-7 !w-12 !px-1.5" type="number" min={0} max={5} step={1} value={interPageWaitMinutes} onChange={(event) => setInterPageWaitMinutes(Number(event.target.value))} /></label>
