...
export function SourceScheduleControl({ source }: { source: CatalogSourceSummary }) {
  ...
  return <div className="grid gap-2 text-xs lg:grid-cols-[auto_minmax(13rem,1fr)_minmax(12rem,1fr)_auto_auto] lg:items-end">
    ...
    <label className="text-muted-foreground">Request ceiling
      <Input aria-label={`Scheduled request ceiling for ${source.displayName}`} className="mt-1 h-8 w-20" type="number" min={2} max={20} step={1} value={requestLimit} disabled={!enabled || !cron} onChange={(event) => setRequestLimit(Number(event.target.value))} />
    </label>
