...
import { getCaptureCandidate, listSourceMediaCandidates, persistListingMedia } from './media.server'
...
export class MediaCaptureError extends Error {
  constructor(message: string, readonly kind: 'access_denied' | 'request_ceiling' | 'persistent_rejection' | 'retry_exhausted' | 'invalid_response') {
    super(message)
...
function assertPublicHttpsUrl(value: string) {
  let url: URL
  try { url = new URL(value) } catch { throw new MediaCaptureError('Image URL is invalid', 'access_denied') }
  if (url.protocol !== 'https:' || url.username || url.password) throw new MediaCaptureError('Image URL must be credential-free HTTPS', 'access_denied')
  ...
  if (url.hostname === 'localhost' || url.hostname.endsWith('.localhost') || /^\d{1,3}(?:\.\d{1,3}){3}$/u.test(url.hostname) || url.hostname.includes(':')) {
    throw new MediaCaptureError('Image URL host is not a permitted public hostname', 'access_denied')
  }
  return url
}
...
async function reserveRequest(budget: MediaRequestBudget, delayMs: number, wait: (milliseconds: number) => Promise<void>) {
  ...
  if (budget.requests > 0) await wait(delayMs)
  budget.requests += 1
}
...
export function createMediaRobotsAccessPolicy(
  ...
  return async (url: string) => {
    ...
    if (cached) return cached.isAllowed(candidate.toString(), userAgent) === true
    await reserveRequest(budget, minimumDelayMs, wait)
    let response: MediaHttpResponse
    try {
      response = await http(`${origin}/robots.txt`, { headers: { accept: 'text/plain', 'user-agent': userAgent }, redirect: 'manual', signal: AbortSignal.timeout(5_000) })
    } catch {
      return false
    }
...
export async function fetchMediaBytes(
  ...
  while (true) {
    ...
    if (response.status === 401 || response.status === 403) throw new MediaCaptureError(`Image access rejected (${response.status})`, 'persistent_rejection')
    if (response.status === 408 || response.status === 425 || response.status === 429 || response.status >= 500) {
      if (retry >= policy.maxRetries) throw new MediaCaptureError(`Image retries exhausted (${response.status})`, 'retry_exhausted')
      retry += 1
      await wait(retryAfterMs(response) ?? policy.minimumDelayMs * 2 ** retry)
      continue
    }
...
export async function captureListingMedia(input: CaptureListingMediaInput) {
  ...
  if (!new Set([candidate.imageUrl, ...candidate.imageUrls].filter((url): url is string => Boolean(url))).has(sourceUrl)) {
    throw new MediaCaptureError('Image URL was not discovered in this listing evidence', 'access_denied')
  }
  if (candidate.moduleId !== 'shopify' || !isAllowedShopifyMediaUrl(candidate.config, sourceUrl)) {
    throw new MediaCaptureError('Image URL is not approved by this source module', 'access_denied')
  }
  ...
  const derivatives = await createImageDerivatives(acquired.bytes)
...
/**
 * Processes a bounded, operator-created batch serially. It deliberately has
 * no automatic continuation, so a long source catalog cannot turn into an
 * unbounded image fan-out.
 */
export async function runMediaCapture(db: Database, runId: string, dependencies: RunMediaCaptureDependencies = {}) {
  ...
  const policy = mediaCapturePolicySchema.parse({ enabled: true, requestLimit: run.requestLimit })
  ...
  try {
    const candidates = (await listSourceMediaCandidates(db, source.id)).slice(0, policy.requestLimit)
    for (const candidate of candidates) {
      try {
        const result = await captureListingMedia({ db, listingId: candidate.listingId, sourceUrl: candidate.sourceUrl, policy, budget, http, accessPolicy, wait: dependencies.wait })
        if (result.status === 'captured') capturedCount += 1
      } catch (captureError) {
        error = captureError instanceof Error ? captureError.message : String(captureError)
        if (captureError instanceof MediaCaptureError && (captureError.kind === 'request_ceiling' || captureError.kind === 'persistent_rejection')) break
      }
      if (budget.requests >= budget.maxRequests) break
    }
    const status = error || budget.requests >= budget.maxRequests ? 'partial' : 'succeeded'
