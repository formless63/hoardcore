...
export const Route = createFileRoute('/listings/')({
  ...
  loader: async () => {
    const [{ listings }, savedViews] = await Promise.all([listCurrentListings(), listSavedListingViews()])
    return { listings, savedViews }
  },
...
function ListingsPage() {
  const { listings, savedViews } = Route.useLoaderData()
  return (
    <main className="w-full px-1 py-2 sm:px-2" id="main-content">
      <h1 className="sr-only">Current listings</h1>
      <CurrentListingsTable listings={listings} savedViews={savedViews} />
    </main>
  )
}
