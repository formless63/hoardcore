...
function Field({ label, value, className = '' }: { label: string; value: string; className?: string }) {
  return <div className={`min-w-0 border-t border-border pt-2 ${className}`}><dt className="text-[11px] uppercase tracking-[.08em] text-muted-foreground">{label}</dt><dd className="mt-0.5 break-words text-sm text-foreground [overflow-wrap:anywhere]">{value}</dd></div>
}
...
function Observation({ listingId, observation }: { listingId: string; observation: ListingDetail['observations'][number] }) {
  ...
  return <article className="border-t border-border py-2 text-xs">
    <div className="flex flex-wrap items-center gap-x-4 gap-y-1">
      ...
      <span>{stockLabel(observation.available, observation.stockQuantity)}</span><span className="min-w-0 flex-[1_1_12rem] truncate" title={observation.title}>{observation.title}</span>
    </div>
...
function ListingDetailPage() {
  ...
  return <main className="w-full px-2 py-3 sm:px-3" id="main-content">
    <div className="flex min-w-0 flex-wrap items-center gap-x-4 gap-y-2"><Link to="/listings" className="mr-auto text-xs text-primary underline">← Listings</Link><div className="flex min-w-0 flex-wrap items-center gap-x-3 gap-y-2"><Await promise={supplement} fallback={<span className="h-6 w-16 animate-pulse rounded bg-muted motion-reduce:animate-none" />}>{({ watched }) => <WatchButton listingId={detail.id} initialWatched={watched} />}</Await><Link to="/research/manual" search={{ listingId: detail.id }} className="text-xs text-primary underline">Add research</Link><a href={detail.url} target="_blank" rel="noreferrer" className="text-xs text-primary underline">Open source ↗</a></div></div>
    <div className="mt-4 min-w-0"><p className="text-xs text-muted-foreground">{detail.source.displayName}</p><h1 className="mt-1 max-w-5xl break-words text-xl font-semibold leading-snug text-foreground [overflow-wrap:anywhere]">{detail.product.title}</h1></div>
    <div className="mt-4 grid min-w-0 gap-4 lg:grid-cols-[minmax(0,20rem)_minmax(0,1fr)] lg:gap-6">
      <div className="min-w-0" aria-label="Captured product images"><CachedListingImage captureId={activeMediaId} variant="preview" alt={detail.product.title} className="flex aspect-square w-full max-w-sm items-center justify-center rounded border border-border bg-card object-contain text-xs text-muted-foreground" />{media.length > 1 ? <div className="mt-2 flex max-w-sm flex-wrap gap-1">{media.map((item, index) => <button key={item.id} type="button" aria-label={`Show captured image ${index + 1}`} aria-pressed={activeMediaId === item.id} onClick={() => setSelectedMediaId(item.id)} className="rounded border border-border aria-pressed:ring-2 aria-pressed:ring-ring"><CachedListingImage captureId={item.id} alt="" className="size-12 object-contain" /></button>)}</div> : null}</div>
      <div className="min-w-0"><dl className="grid min-w-0 grid-cols-2 gap-x-4 gap-y-3 sm:grid-cols-3 lg:grid-cols-4"><Field label="Manufacturer" value={detail.product.brand ?? '—'} /><Field label="Category" value={detail.product.productType ?? '—'} /><Field label="SKU" value={detail.variant.sku ?? '—'} className="col-span-2 sm:col-span-1" /><Field label="Price" value={money(detail.current.price, detail.current.currency)} /><Field label="Was" value={detail.current.compareAtPrice && Number(detail.current.compareAtPrice) > Number(detail.current.price ?? 0) ? money(detail.current.compareAtPrice, detail.current.currency) : '—'} /><Field label="Stock" value={stockLabel(detail.current.available, detail.current.stockQuantity)} /><Field label="Observed" value={detail.current.observedAt.toLocaleString()} className="col-span-2 sm:col-span-1" />{detail.variant.title && detail.variant.title !== 'Default Title' ? <Field label="Variant" value={detail.variant.title} className="col-span-2 sm:col-span-1" /> : null}</dl>
        {detail.product.tags.length ? <p className="mt-3 break-words text-xs text-muted-foreground">Tags: {detail.product.tags.join(', ')}</p> : null}
      </div>
    </div>
    {detail.product.description ? <p className="mt-4 break-words border-t border-border pt-3 text-xs leading-5 text-muted-foreground">{detail.product.description}</p> : null}
    <ListingTrends detail={detail} />
    <OpportunityCalculator listingId={detail.id} price={detail.current.price} currency={detail.current.currency} />
