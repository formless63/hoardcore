...
function AlertSettingsPage() {
  ...
  return <main className="w-full px-2 py-3 sm:px-3" id="main-content">
    ...
    <form className="max-w-xl rounded border border-border bg-card p-3 text-xs" onSubmit={(event) => void submit(event)}>
      ...
      <p className="mt-1 text-muted-foreground">Protected topics use the server-side NTFY_ACCESS_TOKEN when this endpoint matches NTFY_AUTH_ORIGIN. Ask your operator to set both in the app environment; tokens are never entered here.</p>
